...
function SessionItem({
  ...
  const updateMenuPosition = useCallback(() => {
    ...
    setMenuPosition({
      top: rect.bottom + 6,
      left: Math.min(rect.right - menuWidth, viewportWidth - menuWidth - 12),
    });
  }, []);
  ...
  }, [isSubmitting, onRename, renameValue, title]);
  ...
  }, [isSubmitting, onDelete]);

  const renderMenu = () => {
    if (!menuOpen || !menuPosition || typeof document === "undefined") {
      return null;
    }
    ...
    return createPortal(
      <div
        ref={menuRef}
        className="fixed w-32 bg-white rounded-lg shadow-lg border border-black/[0.06] py-1 z-[200] animate-fade-in-scale"
        style={{
      ...
      >
        <button
          ...
          className="w-full flex items-center gap-2 px-3 py-1.5 text-[12px] text-gray-600 hover:bg-gray-50 transition-colors disabled:text-gray-300 disabled:hover:bg-transparent"
        >
        ...
        <button
          ...
          disabled={isSubmitting}
          className="w-full flex items-center gap-2 px-3 py-1.5 text-[12px] text-red-500 hover:bg-red-50 transition-colors disabled:text-red-200 disabled:hover:bg-transparent"
        >
    ...
    return (
      <div className="px-2 py-1 space-y-1">
        <div className="flex items-center gap-1">
          <input
            ref={inputRef}
            className="flex-1 px-2 py-1 text-[13px] rounded-md border border-[#002fa7]/30 bg-white outline-none focus:border-[#002fa7] disabled:bg-gray-50"
            value={renameValue}
            disabled={isSubmitting}
            onChange={(e) => {
          ...
          <button
            ...
            disabled={isSubmitting}
            className="p-1 text-green-600 hover:bg-green-50 rounded disabled:text-gray-300 disabled:hover:bg-transparent"
          >
            {isSubmitting ? (
              <Loader2 className="w-3.5 h-3.5 animate-spin" />
            ) : (
          ...
          <button
            ...
            disabled={isSubmitting}
            className="p-1 text-gray-400 hover:bg-gray-100 rounded disabled:text-gray-300 disabled:hover:bg-transparent"
          >
            <X className="w-3.5 h-3.5" />
          </button>
        </div>
        {error && (
          <p className="px-1 text-[11px] text-red-500 truncate" title={error}>
            {error}
          </p>
        )}
      </div>
      ...
      <div className="relative group">
        ...
        <div className="absolute right-1 top-1/2 -translate-y-1/2">
          <button
            ...
            disabled={isSubmitting}
            className="p-1 rounded-md text-gray-300 opacity-0 group-hover:opacity-100 hover:text-gray-600 hover:bg-black/[0.04] transition-all disabled:opacity-100 disabled:text-gray-300 disabled:hover:bg-transparent"
          >
            {isSubmitting ? (
              <Loader2 className="w-3.5 h-3.5 animate-spin" />
            ) : (
        ...
        {error && !renaming && (
          <p className="px-3 pt-1 text-[11px] text-red-500 truncate" title={error}>
            {error}
          </p>
        )}
      </div>
