      ...
      {!expandedFile && (
        <div className="shrink-0 overflow-y-auto px-1.5 max-h-[35%]">
          {rightTab === "memory" ? (
            ...
            <SkillsFileList skills={skills} onSelect={setInspectorFile} selectedPath={inspectorFile} />
          )}
        </div>
      )}
    ...
    <div className="space-y-0.5">
      ...
      {skills.map((s) => {
        ...
        return (
          ...
          >
            {active && (
              <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-4 bg-[#f59e0b] rounded-r-full" />
            )}
            <Zap className="w-3.5 h-3.5 shrink-0 text-[#f59e0b]" />
            <div className="min-w-0">
              <span className="truncate block">{s.name}</span>
              {s.description && (
                <span className="text-[10px] text-gray-400 truncate block">{s.description}</span>
              )}
            </div>
          </button>
        );
